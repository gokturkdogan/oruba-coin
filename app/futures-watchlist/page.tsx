...
export default function FuturesWatchlistPage() {
  ...
  const wsRef = useRef<WebSocket | null>(null) // Futures ticker WebSocket
  const tradesWsRef = useRef<WebSocket | null>(null) // Futures trade WebSocket
  ...
  // Subscribe to Futures WebSocket for real-time updates
  const subscribeToWebSocket = useCallback((symbols: string[]) => {
    ...
    try {
      ...
      futuresWs.onopen = () => {
        ...
        console.log('Futures Watchlist WebSocket connected')
      }
        ...
        try {
          ...
          if (message.stream && message.data) {
            ...
            if (isMountedRef.current && coinsMapRef.current.has(symbol)) {
              ...
              // EXACTLY like futures-coins: use existingCoin but get latest from map
              ...
              const updatedCoin: Coin = {
                ...
                hourlyFuturesVolume: latestCoin.hourlyFuturesVolume || '0', // Preserve hourly volume from trade WebSocket
                hourlyFuturesBuyVolume: latestCoin.hourlyFuturesBuyVolume || '0', // Preserve hourly buy volume from trade WebSocket
                hourlyFuturesSellVolume: latestCoin.hourlyFuturesSellVolume || '0', // Preserve hourly sell volume from trade WebSocket
              }
  ...
  // Subscribe to Futures Trade WebSocket for hourly volume tracking
  const subscribeToTradeWebSocket = useCallback((symbols: string[]) => {
    ...
    // EXACTLY like futures-coins page - 300ms throttle ensures smooth updates
    ...
    try {
      ...
      futuresTradeWs.onopen = () => {
        clearTimeout(wsTimeout)
        console.log('Futures Trade WebSocket connected (watchlist)')
      }
  ...
  const fetchWatchlist = useCallback(async () => {
    ...
    try {
      ...
      const watchlistRes = await fetch('/api/watchlist/futures')
      if (!watchlistRes.ok) {
      ...
      const coinsRes = await fetch('/api/watchlist/futures/coins')
      if (!coinsRes.ok) {
      ...
      const watchlistCoins = (coinsData.coins || []).map((coin: any) => ({
        ...
        hourlyFuturesVolume: coin.hourlyFuturesVolume || '0',
        hourlyFuturesBuyVolume: coin.hourlyFuturesBuyVolume || '0',
        hourlyFuturesSellVolume: coin.hourlyFuturesSellVolume || '0',
      } as Coin))
  ...
  const fetchAlerts = useCallback(async () => {
    ...
    try {
      const res = await fetch('/api/alerts?market=futures')
      if (res.ok) {
        const data = await res.json()
        ...
        <div className="flex flex-col items-center justify-center min-h-[60vh]">
          <div className="bg-card border border-border rounded-lg p-12 max-w-md w-full text-center shadow-lg">
            ...
            <p className="text-muted-foreground mb-6">
              Vadeli takip listesi özelliği sadece premium üyelerimize özeldir. Coin takibi için premium üyeliğe geçin.
            </p>
        ...
        <div className="relative z-10">
          <div className="mb-8">
            <div className="flex items-center justify-between">
              <div>
                <h1 className="text-4xl font-bold gradient-text mb-2">Vadeli Takip Listesi</h1>
                <p className="text-muted-foreground">Vadeli takip listesindeki coinlerin güncel fiyatları ve hacimleri</p>
              </div>
            </div>
          </div>
        ...
        {watchlist.length === 0 ? (
          <Card className="glass-effect border-white/10 shadow-xl">
            <CardContent className="py-12 text-center">
              ...
              <p className="text-muted-foreground mb-6">
                Vadeli coin listesinden takip etmek istediğiniz coinleri ekleyebilirsiniz
              </p>
              <Button asChild className="bg-gradient-to-r from-primary to-primary/80 hover:from-primary/90 hover:to-primary/70 cursor-pointer">
                <Link href="/futures-coins">Vadeli Coin Listesine Git</Link>
              </Button>
            </CardContent>
          </Card>
        ) : (
              ...
              return (
                ...
                >
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          ...
                          title="Vadeli takip listesinden çıkar"
                        >
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <div>
                      <p className="text-sm text-muted-foreground mb-1">24 Saatlik Vadeli Hacim</p>
                    ...
                    <div>
                      <p className="text-sm text-muted-foreground mb-1">Saatlik Vadeli Hacim</p>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-1 text-xs">
                        <div className="flex items-center justify-between sm:justify-start sm:gap-3 rounded-md border border-green-500/20 bg-green-500/5 px-3 py-2">
                          ...
                          <span className="text-green-300 font-semibold">
                            ${parseFloat(coin.hourlyFuturesBuyVolume || '0').toLocaleString('tr-TR', {
                              maximumFractionDigits: 0,
                            })}
                          </span>
                        </div>
                        <div className="flex items-center justify-between sm:justify-start sm:gap-3 rounded-md border border-red-500/20 bg-red-500/5 px-3 py-2">
                          ...
                          <span className="text-red-300 font-semibold">
                            ${parseFloat(coin.hourlyFuturesSellVolume || '0').toLocaleString('tr-TR', {
                              maximumFractionDigits: 0,
                            })}
                          </span>
                        </div>
          ...
          <DialogContent className="glass-effect border-white/10 max-w-md">
            <DialogHeader>
              ...
              <DialogDescription className="text-muted-foreground">
                {selectedCoin && (
                  <>
                    <strong>{selectedCoin}</strong> için vadeli fiyat alarmı {editingAlert ? 'düzenle' : 'oluştur'}
                  </>
                )}
              </DialogDescription>
            </DialogHeader>
          ...
          <DialogContent className="glass-effect border-white/10">
            <DialogHeader>
              ...
              <DialogDescription className="text-muted-foreground">
                {coinToRemove && (
                  <>
                    <strong>{coinToRemove}</strong> coinini vadeli takip listesinden çıkarmak istediğinize emin misiniz?
                  </>
                )}
              </DialogDescription>
            </DialogHeader>
