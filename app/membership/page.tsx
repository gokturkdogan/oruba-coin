...
export default function MembershipPage() {
  ...
  const [membershipStartFormatted, setMembershipStartFormatted] = useState<string | null>(null)
  const [membershipDurationText, setMembershipDurationText] = useState<string | null>(null)
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (profileData?.user?.subscription?.startedAt) {
          try {
            const startDate = new Date(profileData.user.subscription.startedAt)
            setMembershipStartFormatted(
            ...
            console.error('Premium start date parse error:', error)
            setMembershipStartFormatted(null)
            setMembershipDurationText(null)
          }
        } else {
          setMembershipStartFormatted(null)
          setMembershipDurationText(null)
        ...
        if (profileData?.user?.subscription?.currentPeriodEnd) {
          try {
            const expiryDate = new Date(profileData.user.subscription.currentPeriodEnd)
            const now = new Date()
            const diffMs = expiryDate.getTime() - now.getTime()
            const diffDays = Math.floor(diffMs / (1000 * 60 * 60 * 24))
            setDaysUntilExpiry(diffDays)
          } catch (error) {
            console.error('Expiry date parse error:', error)
  ...
  const isSubscriptionActive = useMemo(() => {
    ...
    return user.subscription.status === 'active' && expiry > new Date()
  }, [user?.subscription])

  const remainingTimeText = useMemo(() => {
    if (!user?.subscription || !isSubscriptionActive) return null
    try {
      return formatDistanceToNow(new Date(user.subscription.currentPeriodEnd), {
        addSuffix: true,
        locale: tr,
      })
    } catch {
      ...
      <div className="relative z-10 space-y-8">
        ...
        <Card className="glass-effect border-white/10 shadow-xl">
          ...
          <CardContent className="pt-6 space-y-4">
            {isSubscriptionActive && daysUntilExpiry !== null && daysUntilExpiry <= 7 && (
              <div
              ...
              >
                <div className="mt-0.5">
                  <AlertTriangle className={`h-5 w-5 ${daysUntilExpiry <= 3 ? 'text-red-300' : 'text-yellow-200'}`} />
                </div>
                  ...
                  <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3">
                    <div>
                      <p className="font-semibold uppercase tracking-wide">
                        {daysUntilExpiry <= 3
                          ? 'Avantajlarınızı Kaybetmek Üzeresiniz!'
                          : 'Premium Avantajlarınızı Kaçırmayın'}
                      </p>
                      <p>
                        Premium ayrıcalıklarınız{' '}
                        <span className="font-medium">
                          {daysUntilExpiry <= 0 ? 'bugün' : `${daysUntilExpiry} gün içinde`}
                        </span>{' '}
                        sona eriyor. Üyeliğinizi yenilemezseniz takip listesi limitleriniz ve alarm haklarınız standart
                        plana dönecek.
                      </p>
                    </div>
                    {daysUntilExpiry <= 3 && (
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
              ...
              <div className="space-y-2">
                ...
                <div className="font-semibold text-lg">
                  {user.subscription?.plan ? user.subscription.plan : 'Premium değil'}
                </div>
              </div>
              ...
              <div className="rounded-lg border border-white/10 bg-muted/10 p-4 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                  ...
                  <p className="text-base font-semibold text-foreground">
                    {membershipStartFormatted} tarihinden beri premium ailesindesiniz.
                  </p>
                  {membershipDurationText && (
                    <p className="text-xs text-muted-foreground mt-1">
                      {membershipDurationText.replace('önce', 'dır').replace('sonra', 'içinde')}
                    </p>
                  )}
                </div>
            ...
            {!isSubscriptionActive && (
              <div className="rounded-lg border border-primary/20 bg-primary/5 p-4 flex items-start gap-3">
                ...
                <div className="space-y-1 text-sm">
                  ...
                  <p className="text-muted-foreground">
                    Premium özelliklere erişmek için plan satın almanız gerekir. Satın alma işlemleri yakında bu
                    sayfaya eklenecek.
                  </p>
                </div>
              </div>
        ...
        {pendingPayment && (
          <Card className="glass-effect border-yellow-500/30 bg-yellow-500/5 shadow-lg">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <CardDescription className="text-yellow-50/70">
                    Talep onaylanana kadar yeni bir talep oluşturamazsınız. Gerekirse iptal edip yeniden
                    gönderebilirsiniz.
                  </CardDescription>
                </div>
              </div>
            </CardHeader>
            <CardContent className="pt-6 space-y-4 text-sm text-yellow-100">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="rounded-lg border border-yellow-500/30 bg-yellow-500/10 p-3">
                  <p className="text-xs uppercase tracking-wide text-yellow-200/80">Plan</p>
                  <p className="text-base font-semibold text-yellow-50">{pendingPayment.plan}</p>
                </div>
                <div className="rounded-lg border border-yellow-500/30 bg-yellow-500/10 p-3">
                  <p className="text-xs uppercase tracking-wide text-yellow-200/80">Tutar</p>
                  <p className="text-base font-semibold text-yellow-50">
                    ₺{pendingPayment.amount.toLocaleString('tr-TR')}
                  </p>
                </div>
                <div className="rounded-lg border border-yellow-500/30 bg-yellow-500/10 p-3">
                  <p className="text-xs uppercase tracking-wide text-yellow-200/80">Oluşturulma</p>
                  <p className="text-base font-semibold text-yellow-50">
                    {new Date(pendingPayment.createdAt).toLocaleDateString('tr-TR')}
                  </p>
                </div>
              </div>
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <p className="text-yellow-100/80">
                  Ödemeyi tamamladıysanız dekontu destek ekibine iletmeyi unutmayın. Farklı bir planla ilerlemek
                  isterseniz önce mevcut talebi iptal etmeniz gerekir.
                </p>
                <Button
                  variant="outline"
                      ...
                      {bankInfoError ? (
                        ...
                        <>
                          ...
                          <ol className="list-decimal list-inside space-y-2">
                            <li>
                              Yukarıdaki IBAN bilgilerine, talep ettiğiniz plan tutarı kadar havale/EFT yapın. Açıklama
                              alanına e-posta adresinizi yazmayı unutmayın.
                            </li>
                            <li>Ödeme dekontunu destek ekibimize iletin.</li>
                            <li>
                              Dekont incelendikten sonra premium süreniz seçtiğiniz plan kadar uzatılır ve size bilgi
                              verilir.
                            </li>
                          </ol>
                          <p className="text-xs text-yellow-200/60">
                            Not: Ödemeyi yaptıktan sonra talebi iptal etmeyin; onay süreci tamamlanana kadar beklemede
                            kalacaktır.
                          </p>
                        </>
                      ) : (
              ...
              <div className="space-y-3">
                <AlertDialogHeader className="space-y-1 text-left">
                  ...
                  <AlertDialogDescription className="text-sm text-red-100/80">
                    Ödemeyi yaptıysanız iade edilmeyecektir. Talebi iptal ederseniz yeniden plan seçerek aynı işlemi
                    baştan yapmanız gerekir.
                  </AlertDialogDescription>
                </AlertDialogHeader>
                ...
                <AlertDialogFooter className="sm:justify-end sm:space-x-2 space-y-0">
                  ...
                  <AlertDialogAction
                    ...
                    className="bg-red-500 text-white hover:bg-red-400 focus:ring-red-500 focus:ring-offset-black disabled:opacity-70"
                  >
          ...
          <Card className="glass-effect border-white/10 shadow-lg">
            ...
            <CardContent className="pt-6 space-y-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                <div className="rounded-lg border border-white/10 bg-gradient-to-br from-primary/10 to-primary/5 p-4 space-y-2">
                  <p className="font-semibold text-primary">Spot Takip Listesi</p>
                  <p className="text-2xl font-bold">
                    {spotWatchlistCount} <span className="text-base text-muted-foreground">/ {WATCHLIST_LIMIT_PER_LIST}</span>
                  </p>
                  <p className="text-muted-foreground">
                    {WATCHLIST_LIMIT_PER_LIST - spotWatchlistCount} coin ekleyebilirsiniz.
                  </p>
                  <Button asChild size="sm" variant="outline" className="text-primary border-primary/30 hover:bg-primary/10">
                    <Link href="/spot-watchlist">Spot takip listesine git</Link>
                  </Button>
                </div>
                <div className="rounded-lg border border-white/10 bg-gradient-to-br from-secondary/10 to-secondary/5 p-4 space-y-2">
                  <p className="font-semibold text-secondary-foreground/80">Vadeli Takip Listesi</p>
                  <p className="text-2xl font-bold">
                    {futuresWatchlistCount}{' '}
                    <span className="text-base text-muted-foreground">/ {WATCHLIST_LIMIT_PER_LIST}</span>
                  </p>
                  <p className="text-muted-foreground">
                    {WATCHLIST_LIMIT_PER_LIST - futuresWatchlistCount} coin ekleyebilirsiniz.
                  </p>
                  <Button asChild size="sm" variant="outline" className="text-secondary-foreground border-secondary/30 hover:bg-secondary/10">
                    <Link href="/futures-watchlist">Vadeli takip listesine git</Link>
                  </Button>
                </div>
              </div>
              <p className="text-xs text-muted-foreground">
                Her liste için maksimum {WATCHLIST_LIMIT_PER_LIST} coin saklanabilir. Gerektiğinde daha az takip edilen
                coinleri çıkararak yeni coinler ekleyebilirsiniz.
              </p>
            </CardContent>
          </Card>
            ...
            <CardContent className="pt-6 space-y-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                <div className="rounded-lg border border-white/10 bg-primary/5 p-4 space-y-2">
                  <p className="font-semibold text-primary">Spot Alarmlar</p>
                  <div className="flex items-baseline gap-2">
                    <span className="text-3xl font-bold">{spotAlerts.filter((alert) => alert.isActive).length}</span>
                    <span className="text-xs uppercase tracking-wide text-muted-foreground">aktif</span>
                  </div>
                  <p className="text-muted-foreground">
                    Pasif/Gerçekleşen: {spotAlerts.filter((alert) => !alert.isActive).length}
                  </p>
                </div>
                <div className="rounded-lg border border-white/10 bg-secondary/5 p-4 space-y-2">
                  <p className="font-semibold text-secondary-foreground/80">Vadeli Alarmlar</p>
                  <div className="flex items-baseline gap-2">
                  ...
                  <p className="text-muted-foreground">
                    Pasif/Gerçekleşen: {futuresAlerts.filter((alert) => !alert.isActive).length}
                  </p>
                </div>
              </div>
              <div className="rounded-lg border border-dashed border-white/20 p-4 text-sm text-muted-foreground space-y-2">
                ...
                <p className="text-xs">
                  Premium kullanıcılar her coin için yukarı/aşağı olmak üzere iki ayrı alarm tanımlayabilir.
                  Gerçekleşen alarmlar pasif durumuna düşer ve geçmişte saklanır.
                </p>
